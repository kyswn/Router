...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  RoutingTableEntry* longest = nullptr;
  std::list<RoutingTableEntry>::const_iterator r_iter=m_entries.begin();
  while(r_iter!=m_entries.end()){
    if((ip&r_iter->mask)==(r_iter->dest&r_iter->mask)){
      //matches
      if(!longest || r_iter->mask>longest->mask)
        longest=(RoutingTableEntry*)&(*r_iter);
